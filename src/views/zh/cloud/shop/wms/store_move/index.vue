...
import { storeMoveList, storeMoveAudit } from '@api/wms/store_move'
export default {
  name: 'StoreMove',
  components: {
    SugarTable,
  ...
  methods: {
    getList() {
      this.loading = true
      if (this.queryParams._movingDate) {
        [this.queryParams.startDate, this.queryParams.endDate] = this.queryParams._movingDate
      }
    ...
    /** 新增按钮操作 */
    handleAdd() {
      this.$router.push({ name: 'StoreMoveAdd' })
    },
    /** 导出按钮操作 */
    handleExport() {
      // 拿到表头
